...
const Validation = function (values, tableForm) {
	this.values = values
...
Object.assign(Validation.prototype, {
	...
	hasFieldError(field, row) {
		const messages = this.tableForm ? (this.messages[row] || {}) : this.messages
	...
	add(field, text, type, row) {
		let messages = this.tableForm ? this.messages[row] : this.messages
	...
	_requiredRow(field, value, row) {
		if (typeof value !== 'undefined' && null === value || '' === value) {
			const label = Options.translate ? Options.translate('validation:required') : 'Required.'
